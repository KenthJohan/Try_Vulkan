...
uint32_t vks_find_queue_family_index (VkPhysicalDevice device, VkSurfaceKHR surface)
{
	uint32_t count = 0;
	vkGetPhysicalDeviceQueueFamilyProperties (device, &count, NULL);
	VkQueueFamilyProperties q [100];
	ASSERT_F (count <= COUNTOF (q), "");
...
int main(int argc, char* argv[])
{
	...
	
	{
		...
		ASSERT_F (r == 0, "SDL_Init failed: %s\n", SDL_GetError ());
	}
